...
func showPenjualanDialog(w fyne.Window, s *state.Session, refreshCallback func(), existingData *models.SellFull, isEditMode bool) {
	...
	itemsTable.OnSelected = func(id widget.TableCellID) {
		if existingData != nil && !isEditMode { // Disable selection for read-only mode
			return
		}
	...
	submitBtn := widget.NewButton("Submit", func() {
		...
		if !isEditMode {
			// Validasi No Nota duplikat (hanya untuk nota baru)
		...
		// Save to database
		if isEditMode {
			err = s.SellRepo.Update(sell) // UPDATE query
		} else {
			err = s.SellRepo.Create(sell) // INSERT query
		}
	...
	if existingData != nil && !isEditMode {
		// Hide edit elements completely for read-only mode
		kodeBarang.Disable()
		namaBarang.Disable()
		qty.Disable()
		addItemBtn.Hide()
	...
	if existingData != nil && !isEditMode {
		// Read-only: show header only, hide item form
		topContent.Add(headerForm)
		topContent.Add(headerFormSeparator)
	} else {
		// New or Edit mode: show both header and item form
		topContent.Add(headerForm)
		topContent.Add(headerFormSeparator)
	...
	// Initial focus (only when item form is visible)
	if !(existingData != nil && !isEditMode) {
...
func PenjualanPage(w fyne.Window, s *state.Session) fyne.CanvasObject {
	...
	table := widget.NewTable(
		...
		func() fyne.CanvasObject {
			...
			text.TextSize = 13
			text.Alignment = fyne.TextAlignCenter
			printBtn := widget.NewButtonWithIcon("", theme.DocumentPrintIcon(), nil)
			printBtn.Importance = widget.LowImportance
			editBtn := widget.NewButtonWithIcon("", theme.DocumentCreateIcon(), nil)
			editBtn.Importance = widget.LowImportance
			return container.NewMax(bg, text, container.NewCenter(container.NewHBox(printBtn, editBtn)))
		},
		func(id widget.TableCellID, cell fyne.CanvasObject) {
			cont := cell.(*fyne.Container)
			bg := cont.Objects[0].(*canvas.Rectangle)
			...
			btnHBox := btnCont.Objects[0].(*fyne.Container)
			printBtn := btnHBox.Objects[0].(*widget.Button)
			editBtn := btnHBox.Objects[1].(*widget.Button)
			...
			if id.Row-1 < len(data) {
				...
				case 5:
					...
					editBtn.OnTapped = func() {
						...
						sell, err := s.SellRepo.GetByID(editBtnID)
						if err != nil {
							dialog.ShowError(err, w)
							return
						}
						showPenjualanDialog(w, s, refreshTable, sell, true)
					}
					editBtn.Show()
				}
			}
	...
	handleKey := func(k *fyne.KeyEvent) {
		...
		case fyne.KeyInsert:
			...
			isDialogOpen = true
			showPenjualanDialog(w, s, func() { isDialogOpen = false; refreshTable() }, nil, false)
		...
		case fyne.KeyE:
			...
			if selectedRow >= 0 && selectedRow < len(data) {
				...
				isDialogOpen = true
				showPenjualanDialog(w, s, func() { isDialogOpen = false; refreshTable() }, sell, true)
			} else {
		...
		case fyne.KeyV:
			...
			if selectedRow >= 0 && selectedRow < len(data) {
				...
				isDialogOpen = true
				showPenjualanDialog(w, s, func() { isDialogOpen = false; refreshTable() }, sell, false)
			} else {
				dialog.ShowInformation("Info", "Pilih nota terlebih dahulu!", w)
			}
		case fyne.KeyDelete:
	...
	footer := canvas.NewText(
		"[Insert] Nota Baru  [E] Edit Nota  [V] Preview Nota  [Del] Void",
		color.White,
	)
